...
Bot.prototype.sendRequest = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if(_this.gameStart === true) {
            ...
            var map = ['m1', 'm2', 'm3', 'm4', 'm5', 'm6'][Math.floor(Math.random())]
            request.post({
                url: _this.URL,
                form: {
                    key: _this.key,
...
Bot.prototype.parseTheData = function() {
    return new Promise(function(resolve, reject) {
        ...
        _this.yourBot = {};
        if(_this.data.hero.id === 1) {
            _this.yourBot = _this.bot1
        }
        if(_this.data.hero.id === 2) {
            _this.yourBot = _this.bot2
        }
        if(_this.data.hero.id === 3) {
            _this.yourBot = _this.bot3
        }
        if(_this.data.hero.id === 4) {
            _this.yourBot = _this.bot4
        }
        ...
        for(var i = 0; i < (tempMapSize * tempMapSize * 2); i += 2) {
            ...
            if(tempMap[i] === "[") {
                _this.map[xPos][yPos] = "t";
                _this.taverns.push([yPos, xPos]);
            }
            if(tempMap[i] === "@") {
                _this.map[xPos][yPos] = tempMap[i + 1];
                _this.bots.push([yPos, xPos]);
            }
            if(tempMap[i] === "$") {
                if(tempMap[i + 1] === "1") {
                    _this.map[xPos][yPos] = "a";
                    _this.bot1mines.push([yPos, xPos]);
                } else if(tempMap[i + 1] === "2") {
                    _this.map[xPos][yPos] = "b";
                    _this.bot2mines.push([yPos, xPos]);
                } else if(tempMap[i + 1] === "3") {
                    _this.map[xPos][yPos] = "c";
                    _this.bot3mines.push([yPos, xPos]);
                } else if(tempMap[i + 1] === "4") {
                    _this.map[xPos][yPos] = "d";
                    _this.bot4mines.push([yPos, xPos]);
                } else {
                    _this.map[xPos][yPos] = "m";
                    _this.freeMines.push([yPos, xPos]);
                }
            }
            counter++;
        ...
        for(var i = 0; i < _this.gridLength; i++) {
            for(var j = 0; j < _this.gridLength; j++) {
                if(_this.map[i][j] === ' ') {
                    _this.useThisMap[i][j] = 0
                } else {
                    _this.useThisMap[i][j] = 1
                };
            }
        }
        // This Creates the Console Display    
        //     if(_this.mode === "training") console.log(bot.map);
        //      else console.log("---------------------------------")
        ...
        table = new Table({
            ...
            head: ['', 'Life', 'Mines', 'Gold']
        })
        for(i = 0; i < 4; i++) {
            var name = _this.data.hero.id - 1 === i ? 'My Bot' : 'Bot ' + (i + 1)
            var obj = {}
            obj[name] = [bot.data.game.heroes[i].life, bot.data.game.heroes[i].mineCount, bot.data.game.heroes[i].gold]
            table.push(obj)
        }
        console.log('')
